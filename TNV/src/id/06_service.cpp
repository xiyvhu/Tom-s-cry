...
long get(char const* key)const{
    ...
    if(error = pthread_mutex_unlock(&g_mutex)){
        logger_error("call pthread_mutex_unlock fail:%s",strerror(eerno));
        return -1;
    }
...
//应答错误
bool error(acl::socket_stream* conn,short errornumb,char const* format,...)const{
    //错误描述
