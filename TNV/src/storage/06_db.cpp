...
int db_c::connect(void){
	MYSQL* mysql = m_mysql; //备份创建出来的m_mysql
	...
	logger_error("connect database fail:%s",mysql_error(m_mysql = mysql));//方便后续再次尝试连接
...
int db_c::get(char const* appid,char const* userid,char const* fileid,std::string& filepath,long long filesize)const{
    ...
    if(cache.get(key,value) == OK){
        std::value<acl::string> size_path = value.split2(";");
        if(size_path.size() == 2){
    ...
    acl::string sql;
    sql.format("SELECT file_path,file_size FREM %s WHERE id='%s';",tablename.c_str(),fileid);
    ...
    if(!res){
        logger_error("result is null:%s,sql:%s",mysql_error(m_mysql),sql.c_ste());
        return ERROR;
    }
...
int db_c::set(char const* appid,char const* userid,char const* fileid,char const* filepath,long long filesize)const{
    ...
    if(!res && mysql_field_count(m_mysql)){
        logger_error("insert database fail:%s,sql:%s",mysql_error(m_mysql),sql.c_ste());
        return ERROR;
    }
...
//删除文件ID
int db_c::del(char const* appid,char const* userid,char const* fileid,char const* filepath,long long filesize)const{
    //先从缓存中删除文件ID
    cache_c cache;
    acl::string key;
    key.format("udi:fid:%s:%s",userid,fileid);
    if(cache.del(key) != OK){
    ...
    MYSQL_RES* res = mysql_store_result(m_mysql);
    if(!res && mysql_field_count(m_mysql)){
        logger_error("delete database fail:%s,sql:%s",mysql_error(m_mysql),sql.c_ste());
        return ERROR;
    }
