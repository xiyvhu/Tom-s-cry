...
#define CMD_STORAGE_DELETE   73//客户机删存储服务器上的文件
...
//| 8        | 1    | 1    | 16     | 156    | 128    |
#define APPID_SIZE   16//应用ID最大字符数（包含结尾空字符）
#define USERID_SIZE 256//用户ID最大字符数（包含结尾空字符）
#define FILEID_SIZE 128//文件ID最大字符数（包含结尾空字符）

//存储服务器加入包包体
typedef struct storage_join_body{
	char sjb_version[STORAGE_VERSION_MAX + 1];    //版本
	char sjb_groupname[STORAGE_GROUPNAME_MAX + 1];//组名
	char sjb_hosstname[STORAGE_HOSTNAME_MAX + 1]; //主机名
	char sjb_port[sizeof(in_port_t)];             //端口号 规避字节对齐可能带来的问题
...
typedef struct storage_beat_body{
	...
	char sbb_hosstname[STORAGE_HOSTNAME_MAX + 1]; //主机名
}storage_beat_body_t;

//这里因学习原因使用了两种方法构建报文，实际应用时请使用其中某一种，以免维护骂娘